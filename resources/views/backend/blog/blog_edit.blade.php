...
@section('content')
    ...
    <div class="page-wrapper">
        ...
        <div id="addForm" class="form-section">
            <form method="POST" action="{{ route('blog.update',$blogEntry->id),}}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
               <label>Image</label>
                <img id="previewImage1" src="{{ asset('storage/' . $blogEntry->image) }}" width="120"
                    style="margin-bottom:10px;">
                    <br>
                <input type="file" name="image" onchange="previewImage(this,'previewImage1')">

                <label>Title</label>    
                <textarea name="title" id="title" >{{ $blogEntry->title}}</textarea>

                 <label>Details</label>
                <textarea name="details" id="details" >{{ $blogEntry->details}}</textarea>
