...
@section('content')
    ...
    <div class="page-wrapper">
        ...
        <div id="addForm" class="form-section">
            <form method="POST" action="{{ route('industry-partners.update',$industryPartner->id),}}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
               <label>Image</label>
                <img id="previewImage1" src="{{ asset('storage/' . $industryPartner->image) }}" width="120"
                    style="margin-bottom:10px;">
                    <br>
                <input type="file" name="image" onchange="previewImage(this,'previewImage1')">
                <label>Name</label>    
                <input type="text" name="name" id="name" value="{{ $industryPartner->name }}">
                <br>
